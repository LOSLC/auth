...
export default function ImageUploadBlock({
  ...
  // Update existing image URL when prop changes
  useEffect(() => {
    if (initialImageUrl) {
  ...
  useEffect(() => {
    if (fileSet && files.length > 0) {
      fileSet(files[0].file as File);
      setExistingImageUrl(null); // Clear existing image when new file is uploaded
    }
